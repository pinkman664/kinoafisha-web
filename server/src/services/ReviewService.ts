...
export class ReviewService {
  ...
  async deleteComment(commentId: number, userId: number, isAdmin: boolean) {
    ...
    if (!isAdmin && comment.user.userId !== userId) throw new Error('Нет доступа');
    await this.commentRepo.remove(comment);
  }

  async setRating(userId: number, movieId: number, value: number) {
    if (value < 1 || value > 10) throw new Error('Оценка должна быть от 1 до 10');
