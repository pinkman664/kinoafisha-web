...
export class UserController {
    async register(req: Request, res: Response) {
        try {
            ...
            if (!login || !password || !email) {
                return res.status(400).json({ message: 'Все поля (логин, пароль, email) обязательны для заполнения' });
            }
    ...
    async updateProfile(req: AuthRequest, res: Response) {
        try {
            ...
            const avatarImage = (req as any).file ? (req as any).file.buffer : undefined;
